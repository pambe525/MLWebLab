...
QUnit.module("initialize", {
    ...
    test("sends ajax request when select button is clicked", function (assert) {
        ...
        initialize();
        $("select[name='data_file']").val('1');
        $('#select_btn').click();  // Returns false;
        sinon.assert.called(mockAjax);
        var expectedArgs = {url: "load_file", data: "data_file=1", dataType: "json", success: displayFileData};
        sinon.assert.calledWith(mockAjax, expectedArgs);
    });
});

QUnit.module("loadSelectedFile", {
    beforeEach: function() { _addWidgetsTo("qunit-fixture"); },
    afterEach: function() {
        $("#qunit-fixture").empty();
    ...
    test("hides glass pane", function (assert) {
        var response = {"error_message": "error"};
        displayFileData(response);
        assert.ok($("#glass_pane").is(":hidden"));
    });

    test("shows MsgBox if error occurs", function (assert) {
        var response = {"error_message": "Error occurred"};
        displayFileData(response);
        assert.ok($("#msg_box").is(":visible"));
        assert.ok($("#msg_text").text(), "Error occurred");
    });

    test("shows data file content if no error", function (assert) {
        var response = {"error_message": null};
        displayFileData(response);
        assert.ok($("#home_container").is(":visible"));
    });

    test("hides displayed MsgBox if no error", function (assert) {
        var response = {"error_message": null};
        $("#msg_box").removeClass("invisible");
        displayFileData(response);
        assert.ok($("#msg_box").hasClass("invisible"));
    });

    test("display date file summary", function (assert) {
        var response = {"error_message": null, "file_name": "file.dat",
            "data_file_rows": 12, "data_file_cols": 3};
        displayFileData(response);
        assert.equal($("#source_file").text(), "file.dat");
        assert.equal($("#source_rows").text(), 12);
        assert.equal($("#source_cols").text(), 3);
    });
});
